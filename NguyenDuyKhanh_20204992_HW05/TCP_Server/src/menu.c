 ...
 * @param file              The file containing account information.
 */
void loginAccount(int *login, const char *buffer_request, int buffer_request_size, char *buffer_result, int buffer_result_size, const FILE *file){
    ...
    if(*login == 1){
        ...
        strcpy(buffer_result, "213");   //Error code logged previously
    }else if(check == 1){
        memset(buffer_result, 0, buffer_result_size);
